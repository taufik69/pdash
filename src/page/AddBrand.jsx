...
import { ImagePlus, Tag, Star } from "lucide-react";
...
export default function AddBrand() {
  ...
  const handleSubmit = (e) => {
    ...
    brandMutate.mutate(payload, {
      ...
      onError: (err) => {
        toast.error(err?.response?.data?.message || "Failed to create brand. Please try again.");
      },
    });
  };
    ...
    <div style={s.page}>
      ...
      <div className="ac-header">
        <div>
          ...
          <h1 className="ac-title">Add Brand</h1>
          <p style={s.titleSub}>Add a new brand to associate incoming products with.</p>
        </div>
      </div>
      ...
      <div style={s.card}>
          ...
          <div style={s.cardTitleWrap}>
            ...
            <div>
              ...
              <div style={s.cardSubtitle}>
                Link logos and branding information
              </div>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
              ...
              <div style={s.col}>
                ...
                <div style={s.fieldGroup}>
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    placeholder="Describe the brand story or niche…"
                    style={s.textarea}
                  />
                </div>
              </div>
          ...
          <div style={s.formFooter}>
            <span style={s.footerHint}>Fields marked <span style={s.required}>*</span> are required</span>
            <button
              type="submit"
              disabled={brandMutate.isPending}
              className="ac-submit-btn"
              style={{ ...s.submitBtn, ...(brandMutate.isPending ? s.submitDisabled : {}) }}
            >
              {brandMutate.isPending ? <><span style={s.spinner} /> Creating…</> : "Create Brand"}
            </button>
          </div>
        </form>
      </div>
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  .ac-title { font-family: 'Playfair Display', serif; font-size: 30px; font-weight: 700; margin: 0 0 4px; color: #f9fafb; letter-spacing: -0.02em; }
  .ac-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 28px; flex-wrap: wrap; gap: 16px; }
  .ac-card-header { display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 14px; padding: 20px 24px; border-bottom: 1px solid #1f2937; }
  .ac-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 32px; }
  .ac-submit-btn:hover:not(:disabled) { opacity: 0.88; }
  @media (max-width: 700px) { .ac-title { font-size: 22px; } .ac-grid { grid-template-columns: 1fr; gap: 0; } }
  @media (max-width: 480px) { .ac-title { font-size: 18px; } }
`;
...
const s = {
  ...
  textarea: { width: "100%", background: "#0d1117", border: "1.5px solid #1f2937", borderRadius: 10, padding: "11px 14px", fontFamily: "'DM Sans', sans-serif", fontSize: 14, color: "#f3f4f6", outline: "none", resize: "none", minHeight: 130, lineHeight: 1.6, transition: "border-color 0.2s" },
  inputError: { borderColor: "rgba(239,68,68,0.5)" },
  errorMsg: { fontSize: 12, color: "#f87171", margin: 0 },
  dropzone: { border: "1.5px dashed #1f2937", borderRadius: 12, padding: "36px 20px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, cursor: "pointer", textAlign: "center", background: "#0d1117", transition: "border-color 0.2s, background 0.2s", minHeight: 180 },
  dropzoneActive: { borderColor: "rgba(217,119,6,0.5)", background: "rgba(217,119,6,0.04)" },
  dropzoneError: { borderColor: "rgba(239,68,68,0.4)" },
  dropIcon: { width: 48, height: 48, borderRadius: 12, background: "rgba(217,119,6,0.1)", border: "1px solid rgba(217,119,6,0.2)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 4 },
  dropText: { fontSize: 14, color: "#9ca3af", margin: 0, lineHeight: 1.5 },
  dropHint: { fontSize: 11, color: "#374151", margin: 0, letterSpacing: "0.04em" },
  ...
  previewImg: { width: 90, height: 90, objectFit: "cover", borderRadius: 9, border: "1.5px solid #1f2937", flexShrink: 0 },
  previewInfo: { display: "flex", flexDirection: "column", gap: 4, flex: 1, minWidth: 0 },
  previewName: { fontSize: 13, fontWeight: 600, color: "#f3f4f6", wordBreak: "break-all" },
  previewSize: { fontSize: 11, color: "#6b7280" },
  removeBtn: { marginTop: 6, background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.25)", color: "#f87171", borderRadius: 7, padding: "5px 12px", fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", width: "fit-content" },
  ...
  submitBtn: { display: "inline-flex", alignItems: "center", gap: 8, background: "#d97706", color: "#fff", border: "none", borderRadius: 11, padding: "11px 28px", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", transition: "opacity 0.2s", whiteSpace: "nowrap" },
